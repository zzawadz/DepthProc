...
#'
#' @details
#' Function originates from an original algorithm proposed by Rousseeuw and Hubert. Let \eqn{ {Z} ^ {n} = {(x_1, y_1), ..., (x_n, y_n)} \subset {{R} ^ {d}} } denotes a sample considered from a following semiparametric model: \eqn{ {{y}_{l}} = {{a}_{0}} + {{a}_{1}}{{x}_{1l}} + ... + {{a}_{(d - 1)l}}{{x}_{(d - 1)l}} + {{\varepsilon }_{l}}, {l = 1, ..., n} }, we calculate a depth of a fit \eqn{ \alpha = (a_{0}, ..., a_{d - 1}) } as \eqn{ RD(\alpha, {{Z} ^ {n}}) = {u \ne 0}\min\sharp {l: \frac{ {{r}_{l}}(\alpha) }{ {{u} ^ {T}}{{x}_{l}} } < 0, l = 1, ..., n} }, where \eqn{ r(\cdot) } denotes the regression residual, \eqn{ \alpha = (a_{0}, ..., a_{d - 1}) }, \eqn{ {u} ^ {T}{x}_{l} \ne 0 }. \code{The deepest regression estimator} \eqn{ DR(\alpha, {{Z} ^ {n}}) } is defined as \eqn{ DR(\alpha, {{Z} ^ {n}}) = {\alpha \ne 0}\,\arg\max\,RD(\alpha, {{Z} ^ {n}}) }
#' 
#' @references
...
#' legend("bottomleft", c("LS", "DeepReg"), fill = c("black", "red"), cex = 1.4,
#'        bty = "n")
#'
#' @export
#'
