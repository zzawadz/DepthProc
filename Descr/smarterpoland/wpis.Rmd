...
knit_hooks$set(chartPar = function(before, options, envir) {
    if (before) par(mar = c(4, 4, .1, .1), pch = 19)  # smaller margin on top and right
})
...
Same funckje głębi są dosyć mało rozpowszechnionym tematem, dlatego też ten wpis nie tyle będzie opisem możliwości pakietu, tylko przedstawieniem czym te funkcje głębi w ogóle są i pokazanie analogi do znanych i lubianych technik analizy danych.
...
Jak wiadomo kwantyle określa się dla wartości z przedziału $[0,1]$. W przypadku odległości Euklidesa problemem jest to, że przyjmuje ona wartości z przedziału $[0,+\inf)$. Można ją jednak łatwo przeskalować od przedziału $[0,1]$, przy pomocy następującego wzoru:
...
Oprócz wykresu głębia-vs-głębia można w oparciu o głębie danych wprowadzić krzywą skali. Pozwala ona na bardzo intuicyjny sposób przedstawienia wielowymiarowej miary rozproszenia.
...
```{r}
norm = mvrnorm(n = 1000, mu = c(0,0), Sigma = 3*diag(2))
mvt = rmvt(n = 1000, sigma = diag(2), df = 5)
