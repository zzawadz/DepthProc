...
Most of the code is written in C++ for additional efficiency. We also use OpenMP to speedup computations with multithreading. The speedup is close to linear in the number of cores --- the timings below were knitted on a 4-core machine, so restricting `depth()` to one thread costs roughly 4x the elapsed time, and to two threads roughly 2x:
